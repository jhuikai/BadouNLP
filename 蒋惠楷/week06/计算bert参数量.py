...
2. transformer层 BERT-Base通常有12层(已修改代码)
    2.1 self-attention 自注意力机制
        2.1.1 Q, K, V 权重矩阵
            3 * [hidden_size, hidden_size] -> 3 * [768, 768]
            参数量: 3 * 768 * 768 = 1766016
...
4. 总参数量计算
    1. Embedding层
        word_embeddings:       16229184
        position_embeddings:   393216
        token_type_embeddings: 1536
        LayerNorm:             1536
        总计: 16229184 + 393216 + 1536 + 1536 = 16625472
2. Transformer层(12层)
    每层包含以下参数: 
        Q, K, V 权重:          1766016
        attention_output 权重: 589824
        LayerNorm:             1536
        intermediate 权重:     2359296
        output 权重:           2359296
        LayerNorm:             1536
        每层的参数量:   1,766,016 + 589,824 + 1,536 + 2,359,296 + 2,359,296 + 1,536 = 7,078,504
        12层的总参数量: 7,078,504 * 12 = 84,942,048
3. Pooler 层: 
    dense 权重: 589,824
    bias: 768
    总计: 589,824 + 768 = 590,592
4. 最终参数总量: 
    Embedding层: 16,625,472
    Transformer层: 84,942,048
    Pooler层: 590,592
    总参数量 = 16,625,472 + 84,942,048 + 590,592 = 102,158,112
'''
